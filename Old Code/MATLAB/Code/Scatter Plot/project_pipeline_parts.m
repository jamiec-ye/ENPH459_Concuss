...
traindir = {'C:\Users\sgarg\Desktop\EEG\test_data_feat'};
...
[features_no_age_correction, dir_numsubj, age, files] = feature_extraction(traindir,parts);  % extract features


%%TODO: read age and append which are not zeros
age(age==0) = 14+round(rand(size(age(age==0)))*3);
r = repmat(age,parts,1);
age=r(:)';
...
%% TEST dataset
[test_features, test_dir_numsubj] = feature_extraction(testdir, parts);  % extract features
